...
function seedData() {
    const demoListings = [
        ...
        },
        {
            ...
            description: 'Family-friendly resort with lush gardens and swimming pool.',
            address: 'Palm Grove, Goa', contact: '+91 11111 22222', map: 'https://maps.google.com'
        },
        {
        ...
        // Restaurants (Cafes)
        {
            id: 'lst_5', type: 'cafe', name: 'Roast & Coast Cafe', price: 8, rating: 4.3,
            ...
            address: 'Alfama, Goa', contact: '+91 55555 66666', map: 'https://maps.google.com'
        },
        ...
        {
            id: 'lst_7', type: 'cafe', name: 'Beachside Bites', price: 12, rating: 4.4,
            location: 'Goa, India', travelerTypes: ['solo','family'],
            images: ['https://images.unsplash.com/photo-1504674900247-0877df9cc836?q=80&w=1200&auto=format&fit=crop'],
            description: 'Quick snacks and fresh juices right by the beach.',
            address: 'Beach Road, Goa', contact: '+91 99999 00000', map: 'https://maps.google.com'
        },
        ...
        {
            id: 'lst_9', type: 'attraction', name: 'Baga Beach', price: 0, rating: 4.8,
            location: 'Goa, India', travelerTypes: ['family','couple','solo'],
            ...
            address: 'Baga, Goa', contact: '+91 44444 55555', map: 'https://maps.google.com'
        },
        {
            id: 'lst_10', type: 'attraction', name: 'Goa Carnival', price: 30, rating: 4.6,
            location: 'Goa, India', travelerTypes: ['family','couple'],
            images: ['https://images.unsplash.com/photo-1501594907352-04cda38ebc29?q=80&w=1200&auto=format&fit=crop'],
            description: 'Annual festival with parades, music, and dance.',
            address: 'Central Goa', contact: '+91 66666 77777', map: 'https://maps.google.com'
        },
        {
            id: 'lst_11', type: 'attraction', name: 'Chapora Fort', price: 15, rating: 4.4,
            location: 'Goa, India', travelerTypes: ['solo','family'],
            images: ['https://images.unsplash.com/photo-1467269204594-9661b134dd2b?q=80&w=1200&auto=format&fit=crop'],
            description: 'Famous fort offering stunning sunset views.',
            address: 'Chapora, Goa', contact: '+91 88888 99999', map: 'https://maps.google.com'
        },
        {
            id: 'lst_12', type: 'cafe', name: 'Roast & Coast Cafe', price: 8, rating: 4.3,
        ...
        {
            id: 'lst_13', type: 'attraction', name: 'Skyline Gardens', price: 25, rating: 4.7,
...
const Actions = {
    ...
    openFilters(el) {
        let type = el.dataset.type || document.querySelector('.tabs .tab.active')?.dataset.tab || 'accommodation';
    ...
    <section class="card">
        <div class="content">
            ...
            <div class="tabs">
                ...
                <div class="tab" data-tab="restaurant">Restaurants</div>
                <div class="tab" data-tab="attraction">Attractions</div>
                <div class="tab" data-tab="utilities">Things to Know</div>
            </div>
            <div class="row wrap" style="gap:12px;align-items:center">
                <input class="input" id="inlineSearch" placeholder="Search name, area, address" />
                <div class="row" style="margin-left:auto;gap:8px">
                    <span class="muted">Sort</span>
                    <select class="input" id="inlineSort" style="max-width:180px">
            ...
            <div class="row" style="gap:10px;margin-top:10px">
                <button class="btn ghost" data-action="openFilters">Filters</button>
            </div>
        </div>
    </section>
...
function mountResultsPage(){
    ...
    window.resultsState = { query: q, sort: 'featured', text: '', filters: { accommodation:{}, restaurant:{}, attraction:{} } };

    const renderRows = arr => arr.length ? arr.map(l => {
        const imgSrc = (l.images && l.images[0]) ? l.images[0] : '';
        return `
        <div class="list-item" style="cursor:pointer" data-action="openListing" data-id="${l.id}">
            <img class="img" style="height:100px" src="${imgSrc}" alt="${l.name}" />
            <div class="content">
                ...
                <div class="row" style="justify-content:space-between">
                    <span class="muted">${l.address}, ${l.location}</span>
                    <span class="badge">$${l.price}</span>
                </div>
            </div>
        </div>`;
    }).join('') : '<div class="empty">No results in this category.</div>';

    let initialTab = 'accommodation';
    const hasAny = {
        accommodation: all.some(i=>i.type==='accommodation'),
        restaurant: all.some(i=>i.type==='cafe'),
        attraction: all.some(i=>i.type==='attraction')
    };
    if (!hasAny[initialTab]) initialTab = hasAny.restaurant ? 'restaurant' : (hasAny.attraction ? 'attraction' : 'utilities');

    mount.innerHTML = `
        <div id="panel-accommodation" class="tab-panel ${initialTab==='accommodation'?'active':''}"></div>
        <div id="panel-restaurant" class="tab-panel ${initialTab==='restaurant'?'active':''}"></div>
        <div id="panel-attraction" class="tab-panel ${initialTab==='attraction'?'active':''}"></div>
        <div id="panel-utilities" class="tab-panel ${initialTab==='utilities'?'active':''}">
            <div class="grid cols-2">
                <div class="card"><div class="content"><h3>Weather & Climate</h3><p>Get current conditions.</p><button class="btn" data-action="getWeather" data-destination="${q}">Get Weather</button></div></div>
                <div class="card"><div class="content"><h3>Visa Requirements</h3><p>Based on your nationality.</p><button class="btn" data-action="getVisaInfo" data-destination="${q}">Check Visa</button></div></div>
                <div class="card"><div class="content"><h3>Transportation</h3><p>Local options and costs.</p><button class="btn" data-action="getTransport" data-destination="${q}">Transport Info</button></div></div>
                <div class="card"><div class="content"><h3>Cultural Tips</h3><p>Customs and safety tips.</p><button class="btn" data-action="getCultureTips" data-destination="${q}">Cultural Guide</button></div></div>
            </div>
            <div id="utilOut" class="spacer"></div>
        </div>
    `;

    function filterAndSort(type){
        const mapType = { accommodation:'accommodation', restaurant:'cafe', attraction:'attraction' };
        let items = window.resultsBase.filter(i=>i.type===mapType[type]);
        const st = window.resultsState;
        const f = st.filters[type] || {};
        if (st.text) items = items.filter(i => (i.name+" "+i.description+" "+i.address).toLowerCase().includes(st.text.toLowerCase()));
        if (f.minPrice != null && f.minPrice !== '') items = items.filter(i => i.price >= Number(f.minPrice));
        if (f.maxPrice != null && f.maxPrice !== '') items = items.filter(i => i.price <= Number(f.maxPrice));
        if (f.minRating != null && f.minRating !== '') items = items.filter(i => i.rating >= Number(f.minRating));
        if (f.traveler) items = items.filter(i => i.travelerTypes.includes(f.traveler));
        switch (st.sort){
            case 'price': items.sort((a,b)=>a.price-b.price); break;
            case 'rating': items.sort((a,b)=>b.rating-a.rating); break;
            case 'popularity': items.sort((a,b)=> (b.rating*100 + b.price) - (a.rating*100 + a.price)); break;
            default: break;
        }
        return items;
    }

    window.rebuildResultsPanels = function(){
        document.getElementById('panel-accommodation').innerHTML = renderRows(filterAndSort('accommodation'));
        document.getElementById('panel-restaurant').innerHTML = renderRows(filterAndSort('restaurant'));
        document.getElementById('panel-attraction').innerHTML = renderRows(filterAndSort('attraction'));
        enhance(mount);
    }

    // initial build and ensure header reflects initial tab
    window.rebuildResultsPanels();
    const initHeader = document.querySelector(`.tabs .tab[data-tab="${initialTab}"]`);
    if (initHeader) {
    ...
    document.querySelectorAll('.tabs .tab').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            document.querySelectorAll('#resultsMount .tab-panel').forEach(p=>p.classList.remove('active'));
            document.getElementById(`panel-${target}`).classList.add('active');
        });
    });

    // wire sort/search
    const sortSelect = document.getElementById('inlineSort');
    if (sortSelect){ sortSelect.addEventListener('change', ()=>{ window.resultsState.sort = sortSelect.value; window.rebuildResultsPanels(); }); }
    const inlineSearch = document.getElementById('inlineSearch');
    if (inlineSearch){ inlineSearch.addEventListener('input', ()=>{ window.resultsState.text = inlineSearch.value; window.rebuildResultsPanels(); }); }

    // override filter modal to include real controls
    window.showFilterModal = function(type){
        const modal = document.createElement('div');
        modal.className = 'modal';
        const f = window.resultsState.filters[type] || {};
        modal.innerHTML = `
            <div class="modal-content">
                <h3>Filter ${type}</h3>
                <div class="filter-options">
                    <label>Min Price <input class="input" id="f-min" type="number" placeholder="0" value="${f.minPrice??''}"></label>
                    <label>Max Price <input class="input" id="f-max" type="number" placeholder="1000" value="${f.maxPrice??''}"></label>
                    <label>Min Rating <input class="input" id="f-rating" type="number" min="0" max="5" step="0.1" placeholder="3.5" value="${f.minRating??''}"></label>
                    <label>Traveler Type
                        <select class="input" id="f-traveler">
                            <option value="">Any</option>
                            <option ${f.traveler==='solo'?'selected':''}>solo</option>
                            <option ${f.traveler==='family'?'selected':''}>family</option>
                            <option ${f.traveler==='couple'?'selected':''}>couple</option>
                        </select>
                    </label>
                </div>
                <div class="modal-actions">
                    <button class="btn" id="applyFilters">Apply</button>
                    <button class="btn ghost" id="cancelFilters">Cancel</button>
                </div>
            </div>`;
        document.body.appendChild(modal);
        modal.querySelector('#cancelFilters').onclick = ()=> modal.remove();
        modal.querySelector('#applyFilters').onclick = ()=>{
            window.resultsState.filters[type] = {
                minPrice: modal.querySelector('#f-min').value,
                maxPrice: modal.querySelector('#f-max').value,
                minRating: modal.querySelector('#f-rating').value,
                traveler: modal.querySelector('#f-traveler').value
            };
            modal.remove();
            window.rebuildResultsPanels();
        };
...
function renderListingDetail(id) {
    ...
    mount.innerHTML = `
        <div class="card">
            ...
            <div class="content">
                <div class="row" style="justify-content:space-between;align-items:flex-start">
                    ...
                    <div class="row" style="gap:8px;align-items:center"><span class="rating">★ ${l.rating}</span><span class="pill">$${l.price}</span></div>
                </div>
                <div class="muted">${l.location} · ${l.type}</div>
                <p>${l.description}</p>
                <div class="row wrap">
        ...
        <div class="card">
            <div class="content">
                <div class="section-title"><h2>Reviews</h2><span class="muted">Recent</span></div>
                <div class="comment"><div class="row" style="justify-content:space-between"><strong>Aria</strong><span class="rating">★★★★★</span></div><div class="muted">“Loved the vibe and location.”</div></div>
                <div class="comment"><div class="row" style="justify-content:space-between"><strong>Dev</strong><span class="rating">★★★★☆</span></div><div class="muted">“Great value and friendly staff.”</div></div>
                <div class="comment"><div class="row" style="justify-content:space-between"><strong>Mei</strong><span class="rating">★★★★☆</span></div><div class="muted">“Rooms were clean, breakfast was nice.”</div></div>
            </div>
        </div>
    `;
    enhance(mount);
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (location.hash.startsWith('#/results')) mountResultsPage();
});
